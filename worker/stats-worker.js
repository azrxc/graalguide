...
// visitor. Cached via the platform Cache API (caches.default), NOT KV - this
// data is a disposable, eventually-consistent mirror of whatever's already
// on Discord, not something needing durable/globally-consistent storage, so
// a per-datacenter edge cache is a better fit and (unlike KV) doesn't count
// against the Workers KV daily operation quota. Trade-off: unlike a KV
// cache, this can't serve stale data if a fresh Discord fetch fails after
// the cache entry expires (each edge location's cache is independent) - on
// a rare Discord API hiccup the feed shows empty for a few minutes instead
// of last-known content. Only text + image attachments are kept (no bot
// token, no other message metadata) since this gets exposed to the public
// via /discord-feed.
